...
class QEMUMonitorProtocol:
    ...
    def send_test(self, cmdline, timeout=30, convert=True):
        end_time = time.time() + timeout
        if convert:
            cmd = self.__build_cmd(cmdline)
        else:
...
if __name__ == "__main__":
    ...
    while start_str=="none":
        ...
        if start_str == "S":
            ...
            ind = 0
            while ind != 100:
                qmp_socket.send_test("htc_zyq id=7 htc_str='status'")
                ind = ind+1
            time_end = time.time()
